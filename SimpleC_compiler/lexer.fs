...
module lexer =
  ...
  let rec private collectIntLiteral nextc input literal litType = 
    if (List.contains nextc ['0'..'9']) then
      collectIntLiteral (nextChar input) input (literal + (string nextc)) litType
    elif nextc = '.' then
      collectIntLiteral (nextChar input) input (literal + (string nextc)) "real_literal:"
    else
      (literal,nextc,litType)
  ...
  let rec private lexer nextc input tokens = 
      ...
      |  _  when List.contains nextc ['0'..'9'] ->
               ...
               let (literal,lookahead, litType) = collectIntLiteral (nextChar input) input (string nextc) "int_literal:"
               lexer lookahead input ((litType + literal) :: tokens)
